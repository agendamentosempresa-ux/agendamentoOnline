// Script Node.js para criar usuário admin no Supabase usando service_role key
// Uso: SUPABASE_URL="https://..." SUPABASE_SERVICE_ROLE_KEY="..." node scripts/create-admin-user.js
...
async function run() {
  ...
  try {
    ...
    if (listError) {
      console.warn('Aviso: não foi possível listar usuários (podemos prosseguir):', listError.message || listError);
    } else if (Array.isArray(existingUsers?.users)) {
      const found = existingUsers.users.find(u => u.email === email);
      if (found) {
        console.log('Usuário já existe no Auth com id:', found.id);
        // garante profile
        const { error: upsertErr } = await supabase.from('profiles').upsert({ id: found.id, email, full_name, role });
